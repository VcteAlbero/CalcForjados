...
st.title("Forjados: Análisis lineal")
...
# Tabla para MApoyos
st.write("### 3. Momentos en Apoyos:")
df_mapoyos = pd.DataFrame({
    "Apoyo": range(1, len(MApoyos) + 1),
    "Momento (kNm)": MApoyos
})
st.dataframe(df_mapoyos)

# Tabla para ley_momentos, ley_cortantes
st.write("### 4. Tabla de Resultados:")
distancia = [i / 100 for i in range(len(ley_momentos))]
df_results = pd.DataFrame({
    "Distancia (m)": distancia,
    "Momentos (kNm)": ley_momentos,
    "Cortantes (kN)": ley_cortantes,
})
...
st.markdown(
    ...
    <footer>
        ...
        Esta es una aplicación docente, no comercial. Si tienes comentarios o preguntas, por favor contacta con: 
        <a href="mailto:valbero@upv.es">valbero@upv.es</a>.
        </p>
    </footer>
    """,
